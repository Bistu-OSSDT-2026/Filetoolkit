...
function onFilesSelected(files: File[]) {
  ...
  if (paths.length > 0) {
    ...
    // File 对象没有 path 时显示文件名
    filePaths.value = files.map(f => f.name);
  }
}
...
<template>
  <div class="page-container">
    ...
    <!-- 执行 -->
    <div class="actions">
      <el-button type="primary" size="large" :disabled="filePaths.length === 0 || !outputDir || status === 'running'" @click="startCompress">
    ...
    <!-- 进度 -->
    <TaskProgress v-if="status !== 'idle'" :progress="progress" :status="status" :message="message" @cancel="cancel" />
...
.file-item { display: flex; justify-content: space-between; align-items: center; padding: 4px 0; font-size: 13px; border-bottom: 1px solid var(--el-border-color-lighter); }
