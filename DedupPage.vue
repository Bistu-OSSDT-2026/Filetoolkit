...
function selectFolder() {
  ...
  input.onchange = () => {
    ...
    if (files && files.length > 0) {
      ...
      if (firstPath) {
        folderPath.value = firstPath.substring(0, firstPath.lastIndexOf("/"));
        folderPath.value = folderPath.value.replace(/\\/g, "/");
      }
    }
    input.remove();
  };
...
.group-count { font-size: 13px; color: var(--el-text-color-secondary); }
...
.group-hint { font-size: 12px; color: var(--el-text-color-secondary); }
.file-row {
  display: flex;
  align-items: center;
  gap: 8px;
...
.file-row.keep-file { background: var(--el-color-success-light-9); }
.keep-badge {
...
.file-path { font-size: 11px; color: var(--el-text-color-secondary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.selected-count { font-size: 14px; color: var(--el-text-color-regular); }
</style>
